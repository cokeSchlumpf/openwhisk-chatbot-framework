...
describe('core-output', () => {
  it('transforms the bot intent, calls the output connector and persists the context', () => {
    ...
    return requireMock.reRequire('./index').main(params)
      .then(result => {
        ...
        chai.expect(invokeStub.getCall(1).args[0].params.message).to.exist;
  ...
  it('transforms the complex bot intent, calls the output connector multipe times and persists the context', () => {
    // create stubs for actual functions
    const invokeStub = sinon.stub()
      .onCall(0).returns(Promise.resolve({
        statusCode: 200,
    ...
    return requireMock.reRequire('./index').main(params)
      .then(result => {
        ...
        chai.expect(invokeStub.getCall(1).args[0].name).to.equal('testpackage/ouput');
        chai.expect(invokeStub.getCall(1).args[0].params.message).to.equal('This is the message');

        chai.expect(invokeStub.getCall(2).args[0].name).to.equal('testpackage/ouput');
        chai.expect(invokeStub.getCall(2).args[0].params.message).to.equal('Hello World');

        chai.expect(invokeStub.getCall(3).args[0].name).to.equal('testpackage/ouput');
        chai.expect(invokeStub.getCall(3).args[0].params.message.foo).to.equal('bar');

        chai.expect(invokeStub.getCall(4).args[0].name).to.equal('testpackage/core-contextpersist');
        chai.expect(invokeStub.getCall(4).args[0].params.payload).to.exist;
        chai.expect(invokeStub.getCall(4).args[0].params.payload.output.sent[0].message).with.lengthOf(4);
